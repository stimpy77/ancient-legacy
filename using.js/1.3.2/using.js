...
using.getXHR = function() {
	var xhr;
	var gen = true;
	if (!using._xhrs) {
		using._xhrs = new Array();
	}
	for (var x in using._xhrs) {
		xhr = using._xhrs[x];
		if (xhr['ready']) {
			gen = false;
			break;
	...
	if (gen) {
		...
		using._xhrs.push({ready: false, xhr:xhr});
	}
	return xhr;
};
using.markXhrReady = function(xhr) {
	if (!using._xhrs) {
		using._xhrs = new Array();
	}
	for (var x in using._xhrs) {
		var _xhr = using._xhrs[x];
		if (xhr === _xhr.xhr) {
			_xhr.ready = true;
			break;
		}
